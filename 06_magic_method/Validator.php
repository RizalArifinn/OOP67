<?php
    ...
    class Validator
    {
    		...
    		foreach ($rules as $item => $ruleset) {
    			...
    			foreach ($ruleset as $rule) {
    				...
    				if ($pos != false) { //pengecekan $pos
                        ...
                        //jika $pos false/tidak true parameter diisi null/kosong
    					$parameter = '';
    				}


    				$methodName = 'validate'.ucfirst($rule); //ucfirst => menjadikan huruf pertama menjadi kapital
        ...
        public function validateRoda($item, $value, $parameter) //method validasi roda
        {

            //pengecekan/validasi roda
            if ($value > $parameter) {
             $this->errors[$item][] = 'Bukan sepeda motor, karena roda lebih dari '.$parameter;

            } else if ($value < $parameter) {
             $this->errors[$item][] = 'Bukan sepeda motor, karena ban kurang dari '.$parameter;
